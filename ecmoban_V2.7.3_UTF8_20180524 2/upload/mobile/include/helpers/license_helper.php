...
function get_shop_license()
{
    ...
    $sql = "SELECT code, value
            FROM " . $global->ecs->table('shop_config') . "
...
function license_login($certi_added = '')
{
    ...
    $certi['certi_url'] = sprintf($global->ecs->url()); // 网店URL
    $certi['certi_session'] = $global->sess->get_session_id(); // 网店SESSION标识
    $certi['certi_validate_url'] = sprintf($global->ecs->url() . 'certi.php'); // 网店提供于官方反查接口
...
function license_reg($certi_added = '')
{
    ...
    $certi['certi_url'] = sprintf($global->ecs->url()); // 网店URL
    $certi['certi_session'] = $global->sess->get_session_id(); // 网店SESSION标识
    $certi['certi_validate_url'] = sprintf($global->ecs->url() . 'certix.php'); // 网店提供于官方反查接口
    $certi['format'] = 'json'; // 官方返回数据格式
    $certi['certificate_id'] = ''; // 网店证书ID
    // 标识
    $certi_back['succ']   = 'succ';
