...
{
    $global = getInstance();
    return $global->ecs->url() . 'respond.php?code=' . $code;
}
...
function get_order_id_by_sn($order_sn, $voucher = 'false')
{
    ...
    if ($voucher == 'true')
    {
        ...
        {
              return $global->db->getOne("SELECT log_id FROM " . $global->ecs->table('pay_log') . " WHERE order_id=" . $order_sn . ' AND order_type=1');
        }
        else
        {
            return "";
    ...
    else
    {
        if(is_numeric($order_sn))
        {
            $sql = 'SELECT order_id FROM ' . $global->ecs->table('order_info'). " WHERE order_sn = '$order_sn'";
            $order_id = $global->db->getOne($sql);
        }
        if (!empty($order_id))
        {
            $pay_log_id = $global->db->getOne("SELECT log_id FROM " . $global->ecs->table('pay_log') . " WHERE order_id='" . $order_id . "'");
            return $pay_log_id;
        }
        else
        {
...
{
    $global = getInstance();
    $sql = 'SELECT goods_name FROM ' . $global->ecs->table('order_goods'). " WHERE order_id = '$order_id'";
    $goods_name = $global->db->getCol($sql);
...
function check_money($log_id, $money)
{
    ...
    {
        $sql = 'SELECT order_amount FROM ' . $global->ecs->table('pay_log') .
              " WHERE log_id = '$log_id'";
        $amount = $global->db->getOne($sql);
    }
    else
    {
        return false;
...
function order_paid($log_id, $pay_status = PS_PAYED, $note = '')
{
    ...
    {
        /* 取得要修改的支付记录信息 */
        $sql = "SELECT * FROM " . $global->ecs->table('pay_log') .
                " WHERE log_id = '$log_id'";
        $pay_log = $global->db->getRow($sql);
        if ($pay_log && $pay_log['is_paid'] == 0)
        {
            /* 修改此次支付操作的状态为已付款 */
            $sql = 'UPDATE ' . $global->ecs->table('pay_log') .
            ...
            if ($pay_log['order_type'] == PAY_ORDER)
            {
                ...
                /* 修改订单状态为已付款 */
                $sql = 'UPDATE ' . $global->ecs->table('order_info') .
                ...
                if (C('sms_order_payed') == '1' && C('sms_shop_mobile') != '')
                {
                    ...
                    $sms = new sms();
                    $sms->send(C('sms_shop_mobile'),
                        sprintf(L('order_payed_sms'), $order_sn, $order['consignee'], $order['tel']),'', 13,1);
                }
            ...
            {
                $sql = 'SELECT `id` FROM ' . $global->ecs->table('user_account') .  " WHERE `id` = '$pay_log[order_id]' AND `is_paid` = 1  LIMIT 1";
                $res_id=$global->db->getOne($sql);
                if(empty($res_id))
                {
                    /* 更新会员预付款的到款状态 */
                    $sql = 'UPDATE ' . $global->ecs->table('user_account') .
                           " SET paid_time = '" .gmtime(). "', is_paid = 1" .
                           " WHERE id = '$pay_log[order_id]' LIMIT 1";
                    $global->db->query($sql);

                    /* 取得添加预付款的用户以及金额 */
                    $sql = "SELECT user_id, amount FROM " . $global->ecs->table('user_account') .
        ...
        else
        {
            ...
            if (!empty($post_virtual_goods))
            {
                $msg = '';
                /* 检查两次刷新时间有无超过12小时 */
                $sql = 'SELECT pay_time, order_sn FROM ' . $global->ecs->table('order_info') . " WHERE order_id = '$pay_log[order_id]'";
